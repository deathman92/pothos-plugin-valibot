...
import createValibotSchema, {
  ...
  createEntriesValidators,
  createObjectValidator,
  isArrayValidator,
  isObjectValidator,
  mergeEntriesValidators,
  refine,
...
> extends BasePlugin<Types> {
  ...
  createValidator(
    ...
    if (type?.kind === "InputObject") {
      ...
      if (options && !isObjectValidator(options)) {
        throw new PothosSchemaError(
          `Expected valid object validator for ${fieldName}`
        );
      }
        ...
        v.lazyAsync(() =>
          v.looseObjectAsync(
            mergeEntriesValidators(
              ...
              (options?.fields && createEntriesValidators(options)) ?? {}
            )
          )
        ),
        options
